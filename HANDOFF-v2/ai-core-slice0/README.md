...
- `src/bakeoff/runBakeoff.ts` — harness: фото×операции×провайдеры → JSON/CSV + scoring-шаблон.
...
```
Появится папка `out/` с результатами, `bakeoff_scoring_template.json` и
`bakeoff_run_manifest.json` с режимом запуска, лимитом и фактическим расходом.
