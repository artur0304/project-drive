...
- локальная продуктовая аналитика считает загрузки, mock-генерации, жалобы и
  публичные ссылки без внешнего трекера и показывает агрегаты в `/admin`.
- пользователь может скачать JSON-экспорт своих проектов, версий, кредитной
  истории и локальной активности без пароля и сессионных токенов.
- каждый ответ backend получает `X-Request-ID`; неожиданные серверные ошибки
  пишутся структурированным JSON без заголовков, тела запроса и текста ошибки.
- `/api/ready` проверяет целостность базы, внешние ключи и применённые миграции
  перед направлением трафика на экземпляр приложения.
